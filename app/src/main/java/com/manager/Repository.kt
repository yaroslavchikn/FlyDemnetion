...
class FileManagerRepository(
    ...
    fun list(dir: File, sortMode: SortMode, ascending: Boolean): List<FileItem> {
        val files = dir.listFiles()?.mapNotNull { file ->
            runCatching { file.toFileItem() }.getOrNull()
        } ?: emptyList()

        return sort(files, sortMode, ascending)
    }

    private fun sort(items: List<FileItem>, mode: SortMode, ascending: Boolean): List<FileItem> {
        ...
        return items.sortedWith(
            compareByDescending<FileItem> { it.isDirectory }.then(finalComparator)
        )
    }

    fun search(dir: File, query: String): List<FileItem> {
        val result = mutableListOf<FileItem>()
        if (query.isBlank()) return result

        searchRecursive(dir, query.trim(), result, 500)
        return result
    }
    ...
    private fun searchRecursive(
        ...
        for (file in children) {
            ...
            if (file.name.contains(query, ignoreCase = true)) {
                out.add(file.toFileItem())
            }

            if (file.isDirectory) {
                searchRecursive(file, query, out, limit)
            }
        }
    }
    ...
    fun restoreAllTrash(): Result<Unit> = runCatching {
        trashItems().forEach { item ->
            restoreTrash(item)
        }
    }

    fun emptyTrash(): Result<Unit> = runCatching {
        permanentDeleteTrash(trashItems())
    }
